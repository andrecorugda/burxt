...
**Released.** 1.7 adds `print_exact` — a String to standard output with **nothing** appended, which
is what a program needs to speak a protocol down its own stdout. LSP framing is `Content-Length: N`,
a blank line, then exactly N bytes; `print` and `print_error` always append a newline, and the routes
that look like they replace them reach a different stream that reorders against `print` and truncates
when stdout is a regular file. A third print statement rather than a builtin, so the per-type
formatter cannot fork.

It also fixes two library defects, both reported against the published 1.6.0 rather than found here.
`json_escape` escaped seven characters and passed the other twenty-five control bytes through raw,
which RFC 8259 §7 forbids — so `json_render` emitted text that is not JSON, and for a zero byte a
string that never closed; all thirty-two now round-trip through a parser that never heard of Burxt.
And `html_escape` wrote `'` as `&#39;` where every other escaper writes `&#x27;` — valid HTML, and
one byte different from the implementation people port from, which is enough to make a port avoid
the library. **If you have a committed page generated by `html_escape`, apostrophes in it move.**

1.6 is the release where every tool in this repository is written in Burxt. The
