...
| **B19** | **The two compilers disagree about what a runtime failure is CALLED**, and nothing compared them. `the_two_compilers_render_a_problem_identically` covers **compile-time** diagnostics; runtime messages were never compared by anything, because the one test that ran `tests/panic/` through stage-1 only asked whether the program died. Four divergences surfaced the instant it began checking the text (v0.0.262): `argument_out_of_range` — stage-1 routes argument access through the array bounds check and loses the framing entirely, saying *"index 99 is outside an array of 1 (at byte 0)"* against stage-0's *"argument(99) does not exist"*, which is **structural, not wording** · `array_oob_runtime` and `slice_index_oob` — *"index 5 is outside an array of 3 (at byte 145)"* against *"index 5 is out of bounds — this array holds 3 values (valid indexes 0 to 2)"* · `mixed_scale_overflow` — stage-1 truncates at *"arithmetic overflow"* and drops the explanation. Held by an **exact-match** list in `the_burxt_backend_keeps_every_runtime_guarantee`, not a count: a new divergence fails, and fixing one fails until it is struck from the list, so neither can be traded for the other. Two are a string, one needs an extra `printf` argument, one needs a distinct check | S–M |
