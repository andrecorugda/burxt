...
- **2.0** — everything in [ROADMAP-2.0](ROADMAP-2.0.md): hosts, and the whole web half — a real
...
| H3 | **MET at 1.0.0.** CI green on `v0.0.297` before the version bump, and green again on the tagged commit — the two-step the v0.0.171 withdrawal taught: a tag on a red commit has to be pulled, so the tag goes on a commit whose CI has already finished rather than on one that is still running. CI green **on the commit being tagged** — a tag on a red commit must be withdrawn, which happened with v0.0.171 |
...
| H9 | **DONE (v0.0.260) — Windows, by container, deliberately.** Windows 11's `wslc` runs OCI images natively — no Docker Desktop, no third-party runtime (preview 29 June 2026, GA fall 2026). So H8's image *is* the Windows host, and the native MSVC port is refused with its bill written out in [ROADMAP-2.0](ROADMAP-2.0.md) §W2. The trigger that reopens it: someone who needs `burxt.exe` outside a container |
