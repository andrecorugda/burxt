...
| An Option-returning GENERIC | `Option<T>` | **NOT A GAP either, and this row is why the one above went wrong twice.** `Option.None` under a type parameter was measured working on 2026-08-16 — `pure function none_of<T>(seed: T) -> Option<T>` compiles, instantiates and runs at a class payload and at a scalar. A3 shipped and neither row was revisited. `array_min<T>` still takes a precondition, which is **better** and should stay, but it is now a choice rather than a workaround, and the row must not go on being read as the reason | **Row retired** |
| A `pure` function that builds an Option | n/a | **NOT A GAP. Measured 2026-08-16 by running it, not by `check`.** `pure` + free generic + `Option.None` and `Option.Some`, at a class payload and at a scalar — four for four, compiled and run. Nothing in that family is refused, and A3 shipped. **This row took three passes to kill and the middle one is the lesson**: the original said "no"; a narrowing said "only under a type parameter"; both were written by someone who had measured the adjacent thing and inferred the rest. **An uncalled generic body typechecks lazily, so `burxt check` is not evidence here — instantiate and run.** The row cost real work: `lib/html.bx` shipped its builders un-`pure` on its authority, and purity being transitive, that would have made every view in every consuming program un-`pure` | **Row retired** |
