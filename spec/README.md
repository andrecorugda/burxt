...
| [M15 Web](M15-WEB.md) | **W0 DONE (2026-08-16); the rest specified, scheduled for 2.0**; its socket foundation shipped early in 1.1.0. `lib/html.bx` and `lib/cgi.bx` both exist, both compile under `the_standard_library_compiles_and_works`, and stage-1 agrees with stage-0 on every fixture. Its *"no template file format"* rule was **superseded** the same day, by Andre, in favour of **BMX** — a markdown-based view format in its own repository, now with two implementations and a conformance suite vendored at `tests/bmx-conformance/`. **The finding that outran the spec: a contract-first view needs NO compiler work** — a view is an ordinary `pure function -> Html` with `requires` on its declaration, and BMX level 2 is a generator, so the compiler does the checking | The web stack as **primitives, not a framework**: `html.bx`, `cgi.bx`, `net.bx`, `http.bx`. Andre's call — 1.0 is the real core and comes first, and the goal is that someone else builds the framework, because that is how a language acquires an ecosystem. **The finding that reorders it: W0 is gated on nothing.** The typed `Html` tree — `Text`/`Raw`/`Element`, escaped on render so an unescaped value is unrepresentable — was **measured compiling and running on today's compiler**, and `cgi.bx` needs only `os_env` and `os_read_all`, which exist. So a Burxt binary behind nginx serves dynamic pages with no socket and no concurrency. Only the listener waits: `sockaddr` needs C struct layouts (unblocked by A7 v0.0.261), and a server needs §G1, where a serial loop and `fork()` were both **refused** in favour of `NOVELTY.md` §6's effect handlers |
