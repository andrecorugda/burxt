...
actually true as of **v0.0.14**, audited by running the compiler — not by
...
| [A5.0 Control Flow](A5.0-CONTROL-FLOW.md) | **Nearly done** (v0.0.3–v0.0.4) | Only `&&`, `\|\|`, `!` with short-circuit are missing. |
...
### A5.0 Control Flow — nearly done, one real gap
...
**The genuine gap: `&&`, `||`, `!` do not exist.** `let c: Bool = a && b;`
fails at the lexer with `unexpected character: '&'`. This is deliverable 3, and
it matters more than its size suggests: without it, any two-condition test
needs nested `if`s, which reads badly. Short-circuit evaluation is
*observable* behavior (§3), so it must be built with basic blocks, not a
bitwise and.
...
1. **Finish A5.0: `&&`, `||`, `!` with short-circuit.** Small, physically
   constrained, no design risk, and it removes a daily readability tax. It
   also closes a milestone rather than opening one.
