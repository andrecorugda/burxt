...
/// `allocates` is worked out rather than written — stage-0's half of M14.
///
/// Its own directory, `tests/stage0-only/`, and the reason is a contract rather than a
/// convenience: every program in `tests/pass/` must be accepted by BOTH compilers, and
/// `the_burxt_backend_compiles_a_growing_share_of_the_suite` asserts 109 of 109. Stage-1
/// still requires the word, so a fixture using the inference cannot live there without
/// either breaking that equality or putting an exemption list on it — and a floor with
/// holes cannot see a regression, which is the whole argument for making it an equality.
///
/// When the stage-1 half lands, the fixture moves to `tests/pass/` and this test goes away.
/// Until then it is the honest place: the feature is real, it is tested, and the test says
/// which compiler it is testing.
#[test]
fn allocates_is_inferred_in_stage_zero() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    ...
    for entry in fs::read_dir(&dir).unwrap() {
        ...
        let got: String = shown.lines().filter(|l| !l.starts_with("compiled ")).map(|l| format!("{}\n", l)).collect();
        assert!(
            out.status.success(),
            "{} no longer compiles:\n{}{}",
            source.display(),
    ...
    let _ = fs::remove_dir_all(&scratch);
    assert!(ran > 0, "tests/stage0-only/ is empty — if the stage-1 half landed, move its \
                      fixtures into tests/pass/ and delete this test rather than leaving \
                      an assertion that passes because it checked nothing");
}
