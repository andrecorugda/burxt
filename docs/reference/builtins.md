...
title: Builtins
section: reference
description: Every call the language owns: what it answers, whether it needs a region, and what it refuses.
---
...
| Call | Answers | Needs a region? | Reaches |
|---|---|---|---|
...
*Needs a region* means the call builds something, and a value has to be built somewhere. You almost never write `region` for it — since v0.0.146 a program has one from the moment it starts, and `allocates` is inferred. See [Memory](../guide/04-memory.html).
...
**Answers** nothing. **Needs a region:** no.
...
**Answers** how many elements, or how many BYTES of a `String`. **Needs a region:** no.
...
**Answers** the byte at `i`. **Needs a region:** no.
...
**Answers** `count` bytes of `s`, starting at `from`. **Needs a region:** yes.
...
Takes an `Int`, a `Bool` or a `Decimal<S>`. It shares its formatter with `print`, so the two can never disagree about what a number looks like. A `Bool` needs no region because both answers are constants.
...
**Answers** the value, written out. **Needs a region:** yes.
...
**Answers** the new length. **Needs a region:** yes.
...
**Answers** the new length. **Needs a region:** no.
...
**Answers** the whole file. **Needs a region:** yes. **Carries** `touches files`, so a caller must declare it.
...
**Answers** how many bytes went out. **Needs a region:** no. **Carries** `touches files`, so a caller must declare it.
...
**Answers** the nth command-line argument. **Needs a region:** no. **Carries** `touches input`, so a caller must declare it.
...
**Answers** how many arguments there are. **Needs a region:** no. **Carries** `touches input`, so a caller must declare it.
...
**Answers** `a / b`, rounded toward negative infinity. **Needs a region:** no.
...
**Answers** `a / b`, rounded toward zero. **Needs a region:** no.
...
**Answers** what is left over. **Needs a region:** no.
...
**Answers** a non-negative hash. **Needs a region:** no.
...
**Answers** what the expression was BEFORE the body ran. **Needs a region:** no.
...
**Answers** the value being returned. **Needs a region:** no.
...
**Answers** nothing — the program ends. **Needs a region:** no.
...
**Answers** nothing — it is not an entry point. **Needs a region:** no.
